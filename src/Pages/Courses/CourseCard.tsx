...
const CourseCard: React.FC<{ details: TCourse }> = ({ details, isLoading }) => {
  ...
  const validReviews = reviews.filter(
    (review) => !isNaN(parseFloat(review.review))
  );
  const averageRating =
    validReviews.length > 0
      ? validReviews.reduce(
          (acc, review) => acc + parseFloat(review.review),
          0
        ) / validReviews.length
      : 0;
  ...
  const starRatings = [...Array(5)].map((_, index) => {
    ...
    return {
      rating,
      count: validReviews.filter(
        (review) => parseFloat(review.review) === rating
      ).length,
    };
  });
  return (
      ...
      >
        ...
        <div className="flex flex-col gap-2">
          ...
          <Link to={""} className="flex justify-between items-center">
            ...
            <img
              className="w-6 group-hover:transform group-hover:-translate-y-0.5 transition duration-300"
              src={arrowUp}
              alt=""
            />
          </Link>
