...
import { useGetBlogByCategoryQuery } from "../../Redux/Features/Blog/blogApi";
import { TBlog, TComment } from "./blog.types";



const BlogDetails = (): JSX.Element => {
  const allBlogs = useLoaderData();
  const {_id, title, category, author, tags, publishedAt, status, blogContent, blogImgUrl, comments } = allBlogs.data;
const {data:relatedBlogsData, isLoading:isRelatedBlogLoading} = useGetBlogByCategoryQuery(category);
  ...
  // Removing the opened blog
  const relatedBlogs = relatedBlogsData?.data?.filter((blog: TBlog) => blog._id !== _id) || [];
  ...
  return (
    <div className="flex flex-col xl:flex-row gap-8 max-w-[1500px] w-[90%] mx-auto mt-5 font-Roboto">
      {/* Blog details */}
      <div className="w-full h-fit xl:w-[70%] flex flex-col gap-5 bg-dark-5/40 border border-dark-10/30 rounded-lg p-4">
        <h1 className="text-neutral-60/80 text-2xl md:text-3xl font-semibold">
          {title}
        </h1>
        ...
        <div className="flex flex-col gap-3 text-body-text font-normal  mt-4">
        ...
        <div className="p-3 border border-dark-10/30 rounded-lg flex flex-col gap-4 mt-3">
        ...
        <div key={comment?._id} className="border-b border-dark-10/50 pb-5 flex flex-col gap-3">
        <div className="flex items-center gap-3">
                <img src={avatar1} alt="" />
                <p>
                  <p className="font-semibold text-neutral-60 text-[15px]">
              ...
              <p className="text-body-text font-normal text-sm">{comment.comment}</p>

              <div className="border border-dark-10/40  w-fit rounded-lg px-3 py-2">
                        <div className="flex gap-3 items-center">
                        <div className="flex items-center gap-1">
                    <img src={like} alt="" className="size-5"/>
                    <p className="text-sm text-gray-500">{comment.appriciated}</p>
                </div>
                ...
                <div className="flex items-center gap-1">
                    <p className="text-sm text-gray-500">{comment.disappreciated}</p>
      ...
      <div className="w-full xl:w-[30%] h-fit bg-dark-5/40 border border-dark-10/30 rounded-lg p-4">
      <h1 className="text-neutral-60/80 text-2xl font-semibold">
          Related Blogs
        </h1>
        <hr  className="border border-dark-10/30 mt-2"/>
       {/* Card */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-1 gap-4 mt-4">
        {
          isRelatedBlogLoading ? "loading" :
          relatedBlogs?.map((blog : TBlog) => (
          <BlogsCard
            key={blog._id}
            _id={blog._id}
